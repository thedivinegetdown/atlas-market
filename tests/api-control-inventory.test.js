...
describe('API control inventory', () => {
  ...
  it('covers every Netlify Function with a known shared wrapper', () => {
    ...
    expect(inventory.summary.total).toBe(280)
    expect(inventory.summary.byWrapper).toEqual({
      'team-authenticated': 8,
      'organization-authenticated': 249,
      authenticated: 21,
      'plain-api': 2,
      unknown: 0,
    })
    expect(inventory.summary.byAccess).toEqual({ read: 80, mutation: 58, 'read-and-mutation': 142 })
    expect(inventory.summary.byPriority).toEqual({ P0: 0, P1: 0, P2: 0, P3: 280 })
    expect(inventory.functions.filter((entry) => entry.wrapper === 'plain-api').map((entry) => entry.function)).toEqual(['health', 'watchlist'])
    expect(inventory.functions).toHaveLength(280)
