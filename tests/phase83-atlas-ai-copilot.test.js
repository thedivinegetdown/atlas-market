...
describe('Phase 83B Atlas Copilot APIs and UI', () => {
  ...
  it('renders lazy-friendly Copilot loading boundary without eager AI panel evaluation', () => {
    const markup = renderToStaticMarkup(React.createElement(App))
    expect(markup).toContain('Atlas Copilot')
    expect(markup).toContain('Loading deferred dashboard feature')
    expect(markup).toContain('Preparing advisory paper-trading context')
    expect(markup).not.toContain('Submit Atlas Copilot question')
    expect(markup).not.toContain('Cancel Atlas Copilot request')
    expect(markup).not.toContain('atlasAi.contextBuilt')
