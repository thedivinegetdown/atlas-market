...
describe('Phase 80B performance, accessibility, and UX hardening', () => {
  it('renders bounded release hardening summaries, accessible labels, status text, and paper-only UX language', () => {
    ...
    expect(markup).toContain('Final Security &amp; UX Hardening')
    expect(markup).toContain('Security policy default-deny checks')
    expect(markup).toContain('keyboard-operable release actions')
    expect(markup).toContain('aria-label="Final Security and UX Hardening"')
    expect(markup).toContain('paper-trading platform readiness only')
    expect(markup).toContain('Viewer access is read-only')
    expect(markup).toContain('summary-only')
    expect(markup).toContain('releaseGate.blocked')
  })
})
