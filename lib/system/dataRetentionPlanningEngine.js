...
export function planDataRetention(input = {}, options = {}) {
  ...
  const backtestReport = input.strategyBacktestReport ?? {}
  const historicalReportUnavailable = [backtestReport.evidenceStatus, backtestReport.releaseResearchRecommendation]
    .some((status) => String(status ?? '').toUpperCase() === 'UNAVAILABLE')
  const backtestRetentionPlanning = retentionPlan(
    'backtest-retention',
    'Backtest retention planning',
    !historicalReportUnavailable && backtestReport.releaseResearchRecommendation ? 'ready' : 'caution',
    backtestReport.eventType,
    'strategy research reports retained by version',
    'future-postgresql-research',
  )
  backtestRetentionPlanning.sourceEvidenceStatus = historicalReportUnavailable ? 'UNAVAILABLE' : backtestReport.evidenceStatus ?? 'NOT_PROVIDED'
