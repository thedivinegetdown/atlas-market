...
export function prepareAiTradingCopilotWorkflowAssistance(input = {}, options = {}) {
  ...
  const historicalEvidenceUnavailable = Number(portfolioInsight.aiTradingCopilotPortfolioInsightSummary?.historicalEvidenceUnavailable ?? 0) > 0
    || Number(conversation.aiTradingCopilotConversationSummary?.historicalEvidenceUnavailable ?? 0) > 0
  ...
  const scoredWorkflowStatus = score >= 85 ? 'ready' : score >= 60 ? 'caution' : 'blocked'
  const workflowStatus = historicalEvidenceUnavailable && scoredWorkflowStatus === 'ready' ? 'caution' : scoredWorkflowStatus
  ...
  const workflows = (sourceItems.length ? sourceItems : [normalizeAiTradingCopilotWorkflowAssistanceRecord({
    ...
    workflowStatus,
    workflowScore: score,
    historicalEvidenceStatus: historicalEvidenceUnavailable ? 'UNAVAILABLE' : 'NOT_PROVIDED',
  ...
  })]).map((item) => normalizeAiTradingCopilotWorkflowAssistanceRecord(
    historicalEvidenceUnavailable
      ? {
          ...item,
          workflowStatus: safeStatus(item.workflowStatus) === 'ready' ? 'caution' : item.workflowStatus,
          workflowScore: score,
          historicalEvidenceStatus: 'UNAVAILABLE',
          workflowSummary: `Copilot workflow score ${score} excludes UNAVAILABLE historical evidence and remains caution for human review.`,
        }
      : item,
  ))
  const aiTradingCopilotWorkflowAssistanceSummary = {
    ...
    historicalEvidenceUnavailable: workflows.filter((item) => item.historicalEvidenceStatus === 'UNAVAILABLE').length,
