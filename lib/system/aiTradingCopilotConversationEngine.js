...
export function prepareAiTradingCopilotConversation(input = {}, options = {}) {
  ...
  const scoredConversationStatus = score >= 85 ? 'ready' : score >= 60 ? 'needs-review' : 'blocked'
  const conversationStatus = historicalEvidenceUnavailable && scoredConversationStatus === 'ready' ? 'needs-review' : scoredConversationStatus
  ...
  })]).map((item) => normalizeAiTradingCopilotConversationRecord(
    historicalEvidenceUnavailable
      ? {
          ...item,
          conversationStatus: safeStatus(item.conversationStatus) === 'ready' ? 'needs-review' : item.conversationStatus,
          conversationScore: score,
          historicalEvidenceStatus: 'UNAVAILABLE',
          portfolioAnalysisSummary: `Portfolio analysis score ${score} excludes UNAVAILABLE historical evidence and requires human review.`,
        }
      : item,
  ))
  const aiTradingCopilotConversationSummary = {
    ...
    historicalEvidenceUnavailable: conversations.filter((item) => item.historicalEvidenceStatus === 'UNAVAILABLE').length,
