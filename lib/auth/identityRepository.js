...
export function createUserSessionRepository({ database, userRepository } = {}) {
  ...
  return {
    ...
    async createSession({ user, session }) {
      ...
      const result = await adapter.query(
        `INSERT INTO atlas_user_sessions (id, user_id, provider, token_hash, status, metadata, refreshed_at, expires_at)
         VALUES ($1, $2, $3, $4, $5, $6, NOW(), $7)
         ON CONFLICT (id)
         DO UPDATE SET status = EXCLUDED.status, metadata = EXCLUDED.metadata, refreshed_at = NOW(), expires_at = EXCLUDED.expires_at
         RETURNING id, user_id, provider, token_hash, status, metadata, created_at, refreshed_at, expires_at, revoked_at`,
        [
    ...
    async refreshSession(sessionId, expiresAt) {
      ...
      const result = await adapter.query(
        'UPDATE atlas_user_sessions SET refreshed_at = NOW(), expires_at = $2 WHERE id = $1 AND status = $3 RETURNING id, refreshed_at, expires_at',
