...
export function createMarketDataService({
  finnhubApiKey = (typeof process !== 'undefined' ? process.env.FINNHUB_API_KEY : '') ?? '',
  twelveDataApiKey = (typeof process !== 'undefined' ? process.env.TWELVEDATA_API_KEY : '') ?? '',
  timeoutMs = 4000,
  ...
  return {
    ...
    async getQuote(symbol, options = {}) {
      ...
      const quote = toMarketServiceQuote(response, metadata.symbol)
      logger.info('market data quote resolved', {
        providerId: quote.provenance?.provider ?? quote.provider,
        symbol: metadata.symbol,
        dataStatus: quote.provenance?.dataStatus ?? 'UNKNOWN',
        fallbackUsed: quote.provenance?.fallbackUsed ?? false,
      })
      return quote
    },
