...
import { normalizeMarketDataProvenance } from './marketDataProvenanceContract.js'

function toServiceQuote(response) {
  const provenance = normalizeMarketDataProvenance({ provider: response.provider, observedAt: response.data?.updatedAt, receivedAt: response.receivedAt, fallbackUsed: response.fallbackUsed, mock: response.mock, warningCodes: response.warningCodes })
  return {
    ...response.data,
    provider: response.provider,
    assetType: response.assetType,
    health: {
      status: provenance.dataStatus === 'LIVE' ? 'healthy' : 'degraded',
      available: true,
      provider: response.provider,
...
function logTwelveDataFailure(logger, symbol, response) {
  if (response?.ok || response?.provider !== 'twelvedata') return
  logger.warn('twelve data provider request failed', {
    event: 'twelve_data_provider_request_failed',
    symbol,
    ...(response.failure ?? {
      operation: 'unknown',
      provider: 'twelvedata',
      ...
      normalizedErrorCode: response.error?.code ?? response.error ?? 'unknown',
      normalizedErrorClass: 'UNKNOWN',
      quotaScope: 'unknown',
      retryAfterSeconds: null,
      resetAt: null,
      resetSeconds: null,
      sanitizedMessageCategory: 'UNKNOWN',
    }),
  })
}
...
export function createDefaultMarketDataProvider({
  ...
  historicalBudget = null,
  creditBudget = createCreditBudget({
    dailyLimit: typeof process !== 'undefined' ? process.env.TWELVEDATA_DAILY_CREDIT_BUDGET : undefined,
    minuteLimit: typeof process !== 'undefined' ? process.env.TWELVEDATA_MINUTE_CREDIT_BUDGET : undefined,
  }),
  ...
  const resolvedHistoricalBudget = historicalBudget ?? creditBudget
  ...
  return {
    ...
    async getQuote(symbol, options = {}) {
      ...
      const providerCandidates = [
        { client: finnhubClient, provider: 'finnhub', configured: Boolean(finnhubApiKey) },
        { client: twelveDataClient, provider: 'twelvedata', configured: Boolean(twelveDataApiKey) },
      ...
      for (const [candidateIndex, candidate] of providerCandidates.entries()) {
        try {
          ...
          if (response?.ok) {
            return { ...normalizeQuoteResponse(response.data, candidate.provider, { assetType }), fallbackUsed: candidateIndex > 0, warningCodes: candidateIndex > 0 ? ['FALLBACK_PROVIDER_USED'] : [] }
          }
          logTwelveDataFailure(logger, normalizedSymbol, response)
        } catch (error) {
      ...
      if (fallback?.ok) {
        logger.warn('market data mock fallback selected', { providerId: fallback.provider, symbol: normalizedSymbol, dataStatus: 'MOCK' })
        return { ...normalizeQuoteResponse(fallback.data, fallback.provider, { assetType }), fallbackUsed: true, mock: true, warningCodes: ['MOCK_FALLBACK_USED'] }
      }
    ...
    async getQuotes(symbols = [], options = {}) {
      ...
      if (symbolCount > 0 && twelveDataApiKey) {
        await creditBudget.waitForCapacity(symbolCount)
      }
      const response = await twelveDataClient.getQuotes(symbols, options)
      if (!response.ok) {
        const fallbackQuotes = await Promise.all(symbols.map(async (symbol) => {
          const fallback = await mockProvider.getQuote(symbol)
          if (!fallback?.ok) return null
          return toServiceQuote({
            ...normalizeQuoteResponse(fallback.data, fallback.provider, { assetType: normalizeAssetType(options.assetType) }),
            fallbackUsed: true,
            mock: true,
            warningCodes: ['MOCK_FALLBACK_USED'],
          })
        }))
        const data = fallbackQuotes.filter(Boolean)
        if (data.length > 0) {
          logger.warn('market data mock fallback selected', { providerId: 'mock', symbolCount: data.length, dataStatus: 'MOCK' })
          return {
            ok: true,
            provider: 'mock',
      ...
      return {
        ...
        provider: providerId,
        assetType: normalizeAssetType(options.assetType),
        data: response.data.map((quote) => toServiceQuote({ ...response, data: quote, provider: response.provider })),
        receivedAt: response.receivedAt,
    ...
    async getCandles(symbol, options = {}) {
      ...
      if (providerBlockedUntil > now) {
        ...
        return {
          ...
          statusCode: 429,
          retryAfterSeconds,
          budget: creditBudget.inspect(),
        }
      }
      const budgetDecision = twelveDataApiKey
        ? resolvedHistoricalBudget.consume(1)
        : { ok: true, budget: resolvedHistoricalBudget.inspect() }
      if (!budgetDecision.ok) {
        logger.warn('historical provider request rejected by process-local credit budget', {
          providerId: 'twelvedata',
          symbol: normalizedSymbol,
        ...
        return {
          ...normalizeProviderError(budgetDecision.code, 'credit budget is exhausted', providerId),
          statusCode: 429,
          retryAfterSeconds: budgetDecision.retryAfterSeconds,
...
export function toMarketServiceQuote(providerResponse, symbol) {
  ...
  const normalizedSymbol = String(symbol ?? '').trim().toUpperCase()
  const provenance = normalizeMarketDataProvenance({ provider: providerResponse?.provider ?? 'unknown', available: false, warningCodes: [providerResponse?.error?.code ?? 'MARKET_DATA_UNAVAILABLE'] })
  return {
    symbol: normalizedSymbol,
    price: 0,
    open: 0,
