...
export function calculateAtrPercentileSeries(candles = [], period = 14, samples = 100) {
  return candles.map((_, index) => {
    const atrSeries = calculateAtr(candles.slice(0, index + 1), period).series
    const sample = atrSeries.slice(-samples)
    return sample.length >= samples ? calculatePercentileRank(sample) : null
  })
}
