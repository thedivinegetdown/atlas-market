...
export function createMarketRegimeOrchestrator({ classifier = classifyMarketRegime, logger } = {}) {
  return {
    classify(context = {}, options = {}) {
      ...
      const result = createRegimeReadModel({ symbol: context.symbol, timeframe, classification, builtInput, marketData: context.marketData })
      logger?.info?.('market regime orchestration completed', {
        ...
        marketDataStatus: result.marketData.dataStatus,
