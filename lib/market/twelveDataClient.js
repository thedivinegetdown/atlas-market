...
function creditHeaders(response) {
  const used = headerValue(response, ['api-credits-used', 'x-api-credits-used', 'x-credits-used'])
  const remaining = headerValue(response, ['api-credits-left', 'api-credits-remaining', 'x-api-credits-left', 'x-credits-left', 'x-ratelimit-remaining'])
  const limit = headerValue(response, ['api-credits-limit', 'x-api-credits-limit', 'x-ratelimit-limit'])
  return {
...
export function createTwelveDataClient({ apiKey, timeoutMs = DEFAULT_TIMEOUT_MS, fetchImpl = globalThis.fetch } = {}) {
  return {
    async getQuote(symbol) {
      ...
      try {
        ...
        return {
          ...
          provider: 'twelvedata',
          receivedAt,
          credits: creditHeaders(response),
          data: {
            symbol,
            price,
            open: quote.open,
    ...
    async getQuotes(symbols = []) {
      if (!apiKey) {
        return { ok: false, error: { code: 'missing_api_key', message: 'Twelve Data API key is not configured' }, provider: 'twelvedata', failure: createFailure({ operation: 'quote_batch', errorCode: 'missing_api_key' }) }
      }
      if (!symbols.length) return { ok: true, provider: 'twelvedata', data: [], receivedAt: new Date().toISOString() }
      ...
      try {
        ...
        const response = await fetchImpl(`https://api.twelvedata.com/quote?symbol=${symbolsParam}&apikey=${apiKey}`, {
          signal: controller.signal,
        })
        ...
        if (!response.ok) {
          const payload = await responsePayload(response)
          const errorCode = Number(payload?.code) === 429 || response.status === 429 ? 'provider_rate_limited' : 'request_failed'
          return { ok: false, error: errorCode, provider: 'twelvedata', status: response.status, failure: createFailure({ operation: 'quote_batch', response, payload, errorCode, message: payload?.message, retryAfter: retryAfterSeconds(response) }) }
        }
        ...
        if (payload?.status === 'error' || Number(payload?.code) >= 400) {
          ...
          return {
            ...
            status: Number(payload?.code) || 400,
            error: errorCode,
            failure: createFailure({ operation: 'quote_batch', response, payload, errorCode, message: payload?.message, retryAfter: retryAfterSeconds(response) }),
          }
        }

        const hasSingleQuote = payload && !Array.isArray(payload?.data) && typeof payload === 'object' && payload.symbol && (payload.price || payload.close)
        ...
        return {
          ...
          error: error?.name === 'AbortError' ? 'timeout' : 'request_failed',
          provider: 'twelvedata',
          failure: createFailure({ operation: 'quote_batch', errorCode: error?.name === 'AbortError' ? 'timeout' : 'request_failed' }),
        }
      }
    },
