...
export function buildAtlasMarketContext({ candleSets = {}, marketRegime = null, selectedSymbol = null, selectedCandles = null, sectorProxySymbol = null, registry = buildMarketContextRegistry(), generatedAt = new Date().toISOString() } = {}) {
  ...
  const sectorRelativeStrength = sectorProxySymbol && selectedReturn != null && sectorReturn != null ? Number((selectedReturn - sectorReturn).toFixed(2)) : 'UNAVAILABLE'
  const sector = leadership.sectors.find((entry) => entry.symbol === sectorProxySymbol)
  const alignmentStatus = !sectorProxySymbol ? 'UNAVAILABLE' : sectorRelativeStrength === 'UNAVAILABLE' ? 'INSUFFICIENT_DATA' : sectorRelativeStrength < 0 || ['LAGGING', 'WEAKENING'].includes(sector?.leadershipStatus) ? 'MISALIGNED' : ['LEADING', 'IMPROVING'].includes(sector?.leadershipStatus) ? 'ALIGNED' : 'NEUTRAL'
  const candidate = selectedSymbol ? { symbol: selectedSymbol, benchmarkSymbol: 'SPY', sectorProxySymbol: sectorProxySymbol ?? null, benchmarkRelativeStrength: selectedReturn != null && spyReturn != null ? Number((selectedReturn - spyReturn).toFixed(2)) : 'UNAVAILABLE', sectorRelativeStrength, alignmentStatus, freshness: selectedCandles?.length ? 'FRESH' : 'UNAVAILABLE' } : null
...
export function createMarketContextService({ marketDataService, registry = buildMarketContextRegistry(), logger } = {}) {
  if (!marketDataService?.getCandles) throw new Error('market data service with daily candle support is required')
  return {
    async refresh({ marketRegime = null, selectedSymbol = null, selectedCandles = null, sectorProxySymbol = null, generatedAt = new Date().toISOString() } = {}) {
      const symbols = registry.refreshSymbols
      const responses = await Promise.all(symbols.map(async (symbol) => ({ symbol, response: await marketDataService.getCandles(symbol, { interval: '1d', limit: 260 }) })))
      const candleSets = Object.fromEntries(responses.map(({ symbol, response }) => [symbol, response?.ok ? response.data : []]))
      const context = buildAtlasMarketContext({ candleSets, marketRegime, selectedSymbol, selectedCandles, sectorProxySymbol, registry, generatedAt })
      const unavailable = responses.filter(({ response }) => !response?.ok).map(({ symbol }) => symbol)
      logger?.info?.('market context refresh completed', { requestCount: symbols.length, unavailableCount: unavailable.length, participationStatus: context.participation.status })
      return Object.freeze({ ...context, refresh: { requestedSymbols: symbols, requestCount: symbols.length, minuteBudgetRequirement: symbols.length, cacheAndDeduplicationDelegatedToMarketDataService: true, unavailableSymbols: unavailable } })
    },
  }
}