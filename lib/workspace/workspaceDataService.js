  ...
  return {
    ...
    async getTradeQuality(candidate, { timeframe = '1D', now } = {}) {
      const [marketOverview, riskResult] = await Promise.all([
        this.getMarketOverview(candidate.symbol, {
          timeframe,
          now,
          includeHistoricalIntelligence: true,
          includeIndicatorEvidence: true,
        }),
        candidate.scannerSource ? this.getRiskSummary(candidate.symbol) : Promise.resolve({ risk: null }),
      ])
      ...
      const scannerSignal = candidate.scannerSource ? signalEngine.evaluateQuote(marketOverview.quote) : null
      const scannerDirection = ['BUY', 'STRONG_BUY'].includes(scannerSignal?.action)
        ? 'bullish'
        : ['SELL', 'AVOID'].includes(scannerSignal?.action) ? 'bearish' : 'neutral'
      const risk = riskResult.risk
      const indicators = marketOverview.indicatorBundle?.indicators ?? {}
      const candles = marketOverview.indicatorBundle?.candles ?? []
      const regime = marketOverview.regime
      const marketContext = { participation: { status: 'MIXED' }, selectedCandidateContext: { alignmentStatus: 'UNAVAILABLE' } }
      const evidenceFreshness = regime.freshness ?? 'FRESH'
