...
import { createMarketEvidencePacketBuilder, GOVERNED_UNIVERSE } from '../market/marketEvidencePacket.js'
...
import { createForwardTestEvidenceRecord } from '../opportunities/forwardTest/forwardTestEvidence.js'
...
import { buildBreakoutMomentumSignal } from '../strategies/breakout/breakoutMomentumSignal.js'
import { buildRangeMeanReversionSignal } from '../strategies/range/rangeMeanReversionSignal.js'
import { buildVolatilityExpansionSignal } from '../strategies/volatility/volatilityExpansionSignal.js'
...
export function createWorkspaceDataService({
  ...
  indicatorPipeline,
  evidencePacketBuilder = createMarketEvidencePacketBuilder({
    marketDataService: defaultMarketDataService,
    regimeOrchestrator: defaultRegimeOrchestrator,
    indicatorPipeline: createDailyIndicatorPipeline({ marketDataService: defaultMarketDataService }),
  }),
} = {}) {
  ...
  return {
    ...
    async getMarketOverview(symbol, { timeframe = '1D', now, includeHistoricalIntelligence = false, includeIndicatorEvidence = false } = {}) {
      ...
      return {
        ...
        quote,
        regime,
        ...(includeIndicatorEvidence ? { indicatorBundle } : {}),
      }
    },
    ...
    async buildMarketEvidencePacket(universe = GOVERNED_UNIVERSE, { now: timestamp } = {}) {
      return evidencePacketBuilder.build(universe, { now: timestamp })
    },

    async getTradeQuality(candidate, { timeframe = '1D', now, marketEvidence } = {}) {
      const generatedAt = now ?? new Date().toISOString()
      const symbol = candidate.symbol
      const evidence = marketEvidence?.symbols?.[symbol]

      // If marketEvidence is provided, use it; otherwise fall back to existing behavior
      let quote, candles, indicators, indicatorBundle, regime, risk, scannerSignal, scannerDirection, suitability, marketContext, evidenceFreshness

      if (evidence) {
        // Use precomputed market evidence packet
        ...
        evidenceFreshness = regime?.freshness ?? 'FRESH'

        // Risk summary uses packet quote (preserves freshness: packet built moments ago)
        riskResult = evidence.quote ? await this.getRiskSummary(symbol, { precomputedQuote: evidence.quote }) : { risk: null }
        risk = riskResult?.risk ?? null

        scannerSignal = candidate.scannerSource && quote ? signalEngine.evaluateQuote(quote) : null
        scannerDirection = ['BUY', 'STRONG_BUY'].includes(scannerSignal?.action)
          ? 'bullish'
          : ['SELL', 'AVOID'].includes(scannerSignal?.action) ? 'bearish' : 'neutral'
        ...
        const [marketOverview, riskResult] = await Promise.all([
          this.getMarketOverview(symbol, {
            timeframe,
            now,
            includeHistoricalIntelligence: true,
            includeIndicatorEvidence: true,
          }),
          candidate.scannerSource ? this.getRiskSummary(symbol) : Promise.resolve({ risk: null }),
        ])
        quote = marketOverview.quote
        candles = marketOverview.indicatorBundle?.candles ?? []
        indicators = marketOverview.indicatorBundle?.indicators ?? {}
        indicatorBundle = marketOverview.indicatorBundle
        regime = marketOverview.regime
        evidenceFreshness = regime?.freshness ?? 'FRESH'
        risk = riskResult?.risk ?? null
        scannerSignal = candidate.scannerSource ? signalEngine.evaluateQuote(quote) : null
        scannerDirection = ['BUY', 'STRONG_BUY'].includes(scannerSignal?.action)
          ? 'bullish'
          : ['SELL', 'AVOID'].includes(scannerSignal?.action) ? 'bearish' : 'neutral'
        suitability = selectStrategiesForRegime({
      ...
      // Unified quote reference for both paths
      const currentQuote = quote

      function buildBaseCandidate(strategyId, strategySignal) {
        const base = {
          ...candidate,
          opportunityId: candidate.opportunityId ?? `scanner-${String(candidate.scannerSource).toLowerCase().replace(/[^a-z0-9]+/g, '-').slice(0, 48)}-${candidate.symbol}-${String(candidate.asOf ?? now ?? currentQuote?.updatedAt).replace(/[^0-9]/g, '').slice(0, 14)}`,
          strategyId,
          direction: strategySignal?.side === 'SHORT' ? 'bearish' : 'bullish',
          deterministicMetrics: {
            trendScore: strategySignal?.side === 'SHORT' ? (scannerSignal?.bearScore ?? null) : (scannerSignal?.bullScore ?? null),
            momentumScore: strategySignal?.side === 'SHORT' ? (scannerSignal?.bearScore ?? null) : (scannerSignal?.bullScore ?? null),
            relativeStrength: indicators.relativeStrengthPct,
            relativeVolume: indicators.relativeVolume,
            atrPercentile: indicators.atrPercentile,
          },
          liquiditySummary: currentQuote?.volume > 0 ? { status: 'observed' } : {},
          riskSummary: risk ? { rewardRiskRatio: risk.rewardRatio } : {},
          orderContext: risk && strategySignal ? {
            assetType: candidate.assetType ?? 'equity',
            side: strategySignal.side === 'SHORT' ? 'short' : 'buy',
            orderType: 'market',
            price: currentQuote?.price,
            stopPrice: strategySignal.side === 'SHORT'
              ? Number((Number(currentQuote?.price) + Math.abs(Number(currentQuote?.price) - Number(risk.stopPrice))).toFixed(2))
              : risk.stopPrice,
            targetPrice: strategySignal.side === 'SHORT'
              ? Number((Number(currentQuote?.price) - Math.abs(Number(risk.targetPrice) - Number(currentQuote?.price))).toFixed(2))
              : risk.targetPrice,
            observedAt: currentQuote?.updatedAt,
          } : null,
          breakoutSignal: strategyId === 'breakout-momentum-v1' ? strategySignal : null,
          rangeMeanReversionSignal: strategyId === 'range-mean-reversion-v1' ? strategySignal : null,
          volatilityExpansionSignal: strategyId === 'volatility-expansion-v1' ? strategySignal : null,
        }
        return base
      }

      function evaluateStrategy(strategyRecord) {
        const strategyId = strategyRecord.strategyId
        const suitabilityItem = suitability.strategies.find(s => s.strategyId === strategyId)
        if (!suitabilityItem || suitabilityItem.decision === 'DISABLED') return null

        let strategySignal = null
        if (strategyId === 'breakout-momentum-v1') {
          strategySignal = buildBreakoutMomentumSignal({
        ...
        } else if (strategyId === 'range-mean-reversion-v1') {
          strategySignal = buildRangeMeanReversionSignal({
            symbol: candidate.symbol,
            currentPrice: currentQuote?.price,
            candles,
            indicatorBundle,
        ...
        } else if (strategyId === 'volatility-expansion-v1') {
          strategySignal = buildVolatilityExpansionSignal({
            symbol: candidate.symbol,
            currentPrice: currentQuote?.price,
            candles,
            indicatorBundle,
        ...
        } else if (strategyId === 'index-pullback-v1') {
          const baseCandidate = candidate.scannerSource ? {
            ...candidate,
            opportunityId: candidate.opportunityId ?? `scanner-${String(candidate.scannerSource).toLowerCase().replace(/[^a-z0-9]+/g, '-').slice(0, 48)}-${candidate.symbol}-${String(candidate.asOf ?? now ?? currentQuote?.updatedAt).replace(/[^0-9]/g, '').slice(0, 14)}`,
            strategyId: 'index-pullback-v1',
            direction: scannerDirection,
            deterministicMetrics: {
              trendScore: scannerDirection === 'bearish' ? scannerSignal?.bearScore : scannerSignal?.bullScore,
              momentumScore: scannerSignal?.strength,
              relativeStrength: indicators.relativeStrengthPct,
              relativeVolume: indicators.relativeVolume,
              atrPercentile: indicators.atrPercentile,
            },
            ...
            orderContext: risk ? {
              assetType: candidate.assetType ?? 'equity',
              side: scannerDirection === 'bearish' ? 'short' : 'buy',
              orderType: 'market',
              price: currentQuote?.price,
              stopPrice: scannerDirection === 'bearish'
                ? Number((Number(currentQuote?.price) + Math.abs(Number(currentQuote?.price) - Number(risk.stopPrice))).toFixed(2))
                : risk.stopPrice,
              targetPrice: scannerDirection === 'bearish'
                ? Number((Number(currentQuote?.price) - Math.abs(Number(risk.targetPrice) - Number(currentQuote?.price))).toFixed(2))
                : risk.targetPrice,
              observedAt: currentQuote?.updatedAt,
            } : null,
          ...
          quality.orderContext = baseCandidate.orderContext ?? null
          const fwdEvidence = createForwardTestEvidenceRecord({
            symbol: baseCandidate.symbol,
            regime: regime?.classification ?? {},
            strategySuitability: suitability,
            tradeQuality: quality,
            entryReferenceContext: {
              opportunityId: baseCandidate.opportunityId,
              scannerSource: baseCandidate.scannerSource,
              referencePrice: currentQuote?.price,
            },
            riskGates: {
              evaluated: Boolean(risk),
              passed: risk?.approved === true,
              blockers: risk?.approved === false ? [risk.reason ?? 'existing risk gate rejected the candidate'] : [],
            },
          })
          return {
            strategyId: 'index-pullback-v1',
            quality,
            forwardTestEvidence: fwdEvidence,
            strategySignal: null,
          }
        }

        if (!strategySignal) return null
        if (strategySignal.suitabilityStatus === 'REJECTED' || strategySignal.suitabilityStatus === 'INSUFFICIENT_DATA' || strategySignal.suitabilityStatus === 'STALE') return null

        const baseCandidate = buildBaseCandidate(strategyId, strategySignal)
        const quality = scoreTradeQuality({
          candidate: baseCandidate,
          regime: regime?.classification ?? {},
          strategySuitability: suitability,
      ...
      const strategyResults = EXISTING_ADAPTIVE_STRATEGY_RECORDS
        .map(evaluateStrategy)
        .filter(Boolean)

      if (strategyResults.length === 0) {
        const fallbackQuality = scoreTradeQuality({
          candidate: {
            ...candidate,
            opportunityId: candidate.opportunityId ?? `scanner-${String(candidate.scannerSource).toLowerCase().replace(/[^a-z0-9]+/g, '-').slice(0, 48)}-${candidate.symbol}-${String(candidate.asOf ?? now ?? marketOverview.quote?.updatedAt).replace(/[^0-9]/g, '').slice(0, 14)}`,
            ...
            liquiditySummary: marketOverview.quote?.volume > 0 ? { status: 'observed' } : {},
            riskSummary: risk ? { rewardRiskRatio: risk.rewardRatio } : {},
            orderContext: null,
          },
          regime: marketOverview.regime,
        ...
        return {
          ...
          forwardTestEvidence: createForwardTestEvidenceRecord({
            symbol: candidate.symbol,
            regime: marketOverview.regime,
            strategySuitability: suitability,
            tradeQuality: fallbackQuality,
            entryReferenceContext: {
              opportunityId: candidate.opportunityId,
              scannerSource: candidate.scannerSource,
              referencePrice: marketOverview.quote?.price,
            },
            riskGates: {
              evaluated: Boolean(risk),
              passed: risk?.approved === true,
      ...
      return {
        ...
        forwardTestEvidence: primary.forwardTestEvidence,
        strategyAttribution: strategyResults.map(r => ({
          strategyId: r.strategyId,
          quality: r.quality,
          forwardTestEvidence: r.forwardTestEvidence,
          strategySignal: r.strategySignal,
          strategyFingerprint: r.strategySignal?.strategyFingerprint ?? null,
          suitabilityStatus: r.strategySignal?.suitabilityStatus ?? 'UNKNOWN',
        })),
      }
    },

    async getDailyBriefing(symbol = 'SPY', { timeframe = '1D', now, reviewedOpportunities = [], durablePaperState, marketOverview: suppliedMarketOverview } = {}) {
      const [marketOverview, portfolioResult, alertResult] = await Promise.all([
        suppliedMarketOverview ?? this.getMarketOverview(symbol, { timeframe, now, includeHistoricalIntelligence: true }),
      ...
      return {
        ...
        briefing: buildDailyBriefing({
          regime: { ...marketOverview.regime, marketData },
          strategySuitability: suitability,
    ...
    async getRiskSummary(symbol, { precomputedQuote } = {}) {
      const quote = precomputedQuote ?? await marketDataService.getQuote(symbol)
      ...
      const positionSize = positionSizingEngine.sizeOrder({
