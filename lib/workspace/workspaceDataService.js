      ...
      function buildBaseCandidate(strategyId, strategySignal) {
        const base = {
          ...
          orderContext: risk && strategySignal ? {
            ...
            orderType: 'market',
            price: marketOverview.quote?.price,
            stopPrice: strategySignal.side === 'SHORT'
              ? Number((Number(marketOverview.quote?.price) + Math.abs(Number(marketOverview.quote?.price) - Number(risk.stopPrice))).toFixed(2))
              : risk.stopPrice,
            targetPrice: strategySignal.side === 'SHORT'
              ? Number((Number(marketOverview.quote?.price) - Math.abs(Number(risk.targetPrice) - Number(marketOverview.quote?.price))).toFixed(2))
              : risk.targetPrice,
            observedAt: marketOverview.quote?.updatedAt,
          } : null,
      ...
      function evaluateStrategy(strategyRecord) {
        ...
        if (strategyId === 'breakout-momentum-v1') {
          strategySignal = buildBreakoutMomentumSignal({
            ...
            currentPrice: marketOverview.quote?.price,
            candles,
            indicatorBundle: marketOverview.indicatorBundle,
            regime,
            marketContext,
