...
export function createWorkspaceDataService({
  ...
  return {
    ...
    async getTradeQuality(candidate, { timeframe = '1D', now, marketEvidence } = {}) {
      ...
      let quote, candles, indicators, indicatorBundle, regime, risk, riskResult, scannerSignal, scannerDirection, suitability, marketContext, evidenceFreshness
      ...
      if (strategyResults.length === 0) {
        const fallbackQuality = scoreTradeQuality({
          candidate: {
            ...candidate,
            opportunityId: candidate.opportunityId ?? `scanner-${String(candidate.scannerSource).toLowerCase().replace(/[^a-z0-9]+/g, '-').slice(0, 48)}-${candidate.symbol}-${String(candidate.asOf ?? now ?? quote?.updatedAt).replace(/[^0-9]/g, '').slice(0, 14)}`,
            ...
            liquiditySummary: quote?.volume > 0 ? { status: 'observed' } : {},
            riskSummary: risk ? { rewardRiskRatio: risk.rewardRatio } : {},
            orderContext: null,
          },
          regime,
