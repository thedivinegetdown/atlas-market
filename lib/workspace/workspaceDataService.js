...
export function createWorkspaceDataService({
  ...
  return {
    ...
    async getTradeQuality(candidate, { timeframe = '1D', now } = {}) {
      ...
      const selectedStrategy = suitability.strategies.find((strategy) => strategy.strategyId === candidate.strategyId)
        ?? suitability.strategies[0]
      const risk = riskResult.risk
      const indicators = marketOverview.indicatorBundle?.indicators ?? {}
      const scannerCandidate = candidate.scannerSource ? {
        ...candidate,
        ...
        strategyId: candidate.strategyId ?? selectedStrategy?.strategyId,
      ...
      quality.orderContext = scannerCandidate.orderContext ?? null
      return {
        paperTrading: true,
        advisoryOnly: true,
        quality,
        forwardTestEvidence: createForwardTestEvidenceRecord({
          symbol: scannerCandidate.symbol,
          regime: marketOverview.regime,
          strategySuitability: suitability,
          tradeQuality: quality,
          entryReferenceContext: {
            opportunityId: scannerCandidate.opportunityId,
            scannerSource: scannerCandidate.scannerSource,
