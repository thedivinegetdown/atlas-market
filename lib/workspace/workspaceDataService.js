...
export function createWorkspaceDataService({
  ...
  return {
    ...
    async getDailyBriefing(symbol = 'SPY', { timeframe = '1D', now, reviewedOpportunities = [] } = {}) {
      ...
      return {
        ...
        briefing: buildDailyBriefing({
          ...
          operations: {
            status: quoteHealth.available === false ? 'DEGRADED' : 'HEALTHY',
            provider: quoteHealth.provider ?? marketOverview.quote?.provider ?? 'unknown',
            providerStatus: quoteHealth.available === false ? 'DEGRADED' : 'HEALTHY',
          },
        }, { logger: serverLogger }),
      }
    },
    ...
    async evaluateAlerts(context = {}) {
      const alerts = repositories.alertRepository.list()
