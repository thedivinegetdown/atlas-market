...
export function createForwardEvidenceSnapshot({ manifest, evidence = {}, tradeQuality = {}, entryContext = {} } = {}) {
  ...
  const core = {
    ...
    manifestFingerprint: manifest.manifestFingerprint,
    evaluationId: entryContext.evaluationId ?? null,
    evaluationEvidenceFingerprint: entryContext.evaluationEvidenceFingerprint ?? null,
...
export function buildForwardObservationStatus({ manifest, manifestStatus = 'collecting', snapshots = [], outcomes = [], performanceReview = {}, learningEvidence = {}, currentConfiguration } = {}) {
  ...
  const experimentId = manifest.experiment?.experimentId ?? 'EDGE.2'
  const scopedSnapshots = snapshots.filter((item) => item.experimentId === experimentId && item.observationId === manifest.observationId && item.manifestFingerprint === manifest.manifestFingerprint)
  const scopedOutcomes = outcomes.filter((item) => {
    const cohort = item.forwardObservation ?? item.payload?.forwardObservation
    return cohort?.experimentId === experimentId
      && cohort.observationId === manifest.observationId
      && cohort.manifestFingerprint === manifest.manifestFingerprint
  })
  const sessionsElapsed = tradingSessions(scopedSnapshots, scopedOutcomes)
  const completedOutcomes = scopedOutcomes.filter((item) => {
    const attribution = item.exitAttribution ?? item.payload?.exitAttribution
    return (item.executionType === 'close' || item.accountingStatus === 'position_closed')
      && attribution?.policyCompliant === true
      && attribution.countsTowardObservationMinimum === true
  }).length
  ...
  const dataQualityDegraded = scopedSnapshots.some((item) => item.quoteFreshness !== 'LIVE' || /mock/i.test(item.provider))
  return {
    version: FORWARD_OBSERVATION_VERSION,
    observationId: manifest.observationId,
    experimentId: manifest.experiment?.experimentId ?? 'EDGE.2',
