...
export async function runForwardObservation({ evidenceRepository, ledgerRepository, tenantContext, accountId, userId, now = new Date().toISOString() } = {}) {
  ...
  for (const experiment of SUPPORTED_FORWARD_OBSERVATION_EXPERIMENTS) {
    ...
    if (experiment.experimentId === 'EDGE.2' && (!evaluation.evaluationId || !evaluation.evidenceFingerprint)) {
      details.set(experiment.experimentId, { reason: 'durable_evaluation_linkage_missing' })
      continue
    }
    ...
    const snapshot = createForwardEvidenceSnapshot({
      ...
      entryContext: {
        ...
        evaluationEvidenceFingerprint: evaluation.evidenceFingerprint ?? null,
