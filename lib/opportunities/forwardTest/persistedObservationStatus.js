...
export async function resolvePersistedForwardObservationStatuses({ evidenceRepository, ledgerRepository, tenantContext, accountId, userId, executions } = {}) {
  ...
  const loadedExecutions = typeof ledgerRepository?.listForwardObservationExecutions === 'function'
    ? await ledgerRepository.listForwardObservationExecutions(scope)
    : executions ?? await ledgerRepository?.listExecutions?.(scope) ?? []
