...
function rankPopulation(plans, population, portfolio) {
  ...
  return plans
    ...
    .filter((entry) => entry.ranking?.rankingStatus === 'ranked')
...
export function rankQualifiedTradePlans({ plans = [], portfolioEvidence } = {}) {
  ...
  return Object.freeze({
    version: QUALIFIED_OPPORTUNITY_RANKING_VERSION,
    qualified: rankPopulation(qualifiedPlans, 'QUALIFIED', portfolio),
    watch: rankPopulation(watchPlans, 'WATCH', portfolio),
