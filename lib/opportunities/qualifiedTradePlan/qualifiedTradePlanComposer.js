...
function decisionStatus({ evaluation = {}, tradeQuality = {}, strategySuitability = {}, regime = {}, riskGate = {}, quantity }) {
  ...
  const strategy = strategySuitability.decision ?? evaluation.strategySuitability?.decision ?? evaluation.breakoutSignal?.suitabilityStatus ?? evaluation.rangeMeanReversionSignal?.suitabilityStatus ?? evaluation.volatilityExpansionSignal?.suitabilityStatus
...
export function composeQualifiedTradePlan({ candidate = {}, tradeQuality = {}, regime = {}, strategySuitability = {}, evaluation = {}, riskGate = {}, sizing = {}, strategyVersion = null, policyFingerprint = null, strategyFingerprint = null } = {}, options = {}) {
  ...
  const range = evaluation.rangeMeanReversionSignal ?? candidate.rangeMeanReversionSignal ?? null
  const volatility = evaluation.volatilityExpansionSignal ?? candidate.volatilityExpansionSignal ?? null
  ...
  return Object.freeze({
    ...
    strategyId,
    strategyVersion,
    strategyFamily: strategyId === 'breakout-momentum-v1' ? 'breakout-momentum' : strategyId === 'range-mean-reversion-v1' ? 'range-mean-reversion' : strategyId === 'volatility-expansion-v1' ? 'volatility-expansion' : strategyId === 'index-pullback-v1' ? 'trend-pullback' : null,
    generatedAt,
    ...
    rangeMeanReversion: range ? { prior20Low: range.prior20Low ?? null, currentPrice: range.currentPrice ?? structure.entry, sma20: range.SMA20 ?? null, atr14: range.ATR14 ?? null, stretchAtr: range.stretchAtr ?? null, adx14: range.ADX14 ?? null, rsi14: range.RSI14 ?? null, relativeVolume: range.relativeVolume ?? null, relativeStrength: range.relativeStrength ?? null, marketParticipation: range.marketParticipation ?? null, sectorAlignment: range.sectorAlignment ?? null, evidenceFreshness: range.evidenceFreshness ?? freshness } : null,
    volatilityExpansion: volatility ? { prior20High: volatility.prior20High ?? null, currentPrice: volatility.currentPrice ?? structure.entry, atr14: volatility.ATR14 ?? null, atrPct: volatility.atrPct ?? null, atrPercentile: volatility.atrPercentile ?? null, compressionCount: volatility.compressionCount ?? 0, compressionWindow: volatility.compressionWindow ?? 5, compressionConfirmed: volatility.compressionConfirmed === true, expansionPercent: volatility.expansionPercent ?? null, sma20: volatility.SMA20 ?? null, sma50: volatility.SMA50 ?? null, sma200: volatility.SMA200 ?? null, adx14: volatility.ADX14 ?? null, rsi14: volatility.RSI14 ?? null, relativeVolume: volatility.relativeVolume ?? null, relativeStrength: volatility.relativeStrength ?? null, marketParticipation: volatility.marketParticipation ?? null, sectorAlignment: volatility.sectorAlignment ?? null } : null,
    strategyEvidence: breakout ? { type: 'breakout-momentum', data: breakout } : range ? { type: 'range-mean-reversion', data: range } : volatility ? { type: 'volatility-expansion', data: volatility } : null,
    ...
    decision: { status, supportingReasons, cautionReasons, noTradeReason: status === 'NO_TRADE' ? (planQuantity === 0 ? 'Risk sizing allowed zero quantity.' : evaluation.noActionableSetup === true || tradeQuality.noActionableSetup === true ? 'No actionable setup is present.' : 'The deterministic decision gates did not qualify the setup.') : null },
    empiricalEvidence: { status: 'UNAVAILABLE', reason: 'Empirical forward evidence is not derived before INTEL.6.' },
    integrity: { strategyFingerprint: strategyFingerprint ?? breakout?.strategyFingerprint ?? range?.strategyFingerprint ?? volatility?.strategyFingerprint ?? null, policyFingerprint, experimentId: evaluation.experimentId ?? candidate.experimentId ?? null, evidenceFingerprint: evaluation.evidenceFingerprint ?? tradeQuality.evidenceFingerprint ?? null, sourceReferences: unique([candidate.opportunityId, evaluation.evaluationId]), engineVersions: evaluation.engineVersions ?? { tradeQuality: tradeQuality.engineVersion ?? null, regime: regime.engineVersion ?? null, strategySuitability: strategySuitability.engineVersion ?? null } },
    executable: false,
