...
function finite(value) { if (value == null || value === '') return null; const parsed = Number(value); return Number.isFinite(parsed) ? parsed : null }
...
export function normalizeTradeQualitySnapshot(input = {}) {
  ...
  const evidenceFingerprint = /^[a-f0-9]{64}$/i.test(String(input.evidenceFingerprint ?? '')) ? String(input.evidenceFingerprint).toLowerCase() : null
  ...
  return {
    ...
    qualityStatus: boundedText(input.status ?? input.qualityStatus ?? 'INSUFFICIENT_DATA', 40).toUpperCase(), evidenceFingerprint, reasons: boundedList(input.reasons), blockers: boundedList(input.blockingReasons ?? input.blockers), missingInputs: boundedList(input.missingInputs, 10), freshness: boundedText(input.freshness ?? 'UNKNOWN', 20).toUpperCase(), asOf: asOf.toISOString(), reviewState, engineVersion: 'trade-quality-v1',
    orderContext: input.orderContext && typeof input.orderContext === 'object' ? {
      assetType: boundedText(input.orderContext.assetType, 40).toLowerCase(), side: boundedText(input.orderContext.side, 10).toLowerCase(),
      orderType: boundedText(input.orderContext.orderType ?? 'market', 20).toLowerCase(), price: finite(input.orderContext.price),
      stopPrice: finite(input.orderContext.stopPrice), targetPrice: finite(input.orderContext.targetPrice), quantity: finite(input.orderContext.quantity),
      observedAt: input.orderContext.observedAt ? boundedText(input.orderContext.observedAt, 40) : null,
    } : null,
