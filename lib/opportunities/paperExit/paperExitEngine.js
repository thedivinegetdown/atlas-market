...
export const PAPER_EXIT_VERSION = 'paper-exit-v2'
export const POLICY_EXIT_EVIDENCE_BLOCKER = 'authoritative_exit_chronology_unavailable'
...
// Pure calculation only: policyBar/sessionsHeld are useful for controlled validation,
// but are not authority to persist a compliant outcome. commitExit owns that gate.
export function simulatePaperPositionExit({ position, positions, account = {}, quantity, quote = {}, existingExits = [], paperModeEnabled = true, exitReason, policyBar, sessionsHeld } = {}, options = {}) {
  ...
  const barAge = Date.parse(now) - Date.parse(policyBar?.observedAt ?? policyBar?.timestamp)
  const values = [policyBar?.open, policyBar?.high, policyBar?.low, policyBar?.close].map(Number)
  const [open, high, low, close] = values
  const validBar = policyBar?.freshness === 'FRESH' && Number.isFinite(barAge) && barAge >= 0 && barAge <= config.maxPriceAgeMs
    && values.every(positive) && high >= Math.max(open, close) && low <= Math.min(open, close) && low <= high
    && Date.parse(policyBar.observedAt ?? policyBar.timestamp) >= Date.parse(position.exitPolicy?.enteredAt)
  const policyDecision = position.exitPolicy && !emergency
    ? validBar ? evaluatePolicy({ policy: position.exitPolicy, bar: policyBar, sessionsHeld }) : { action: 'BLOCKED', reason: 'missing_or_stale_market_evidence' }
    : null
  ...
  const fillReference = exitAttribution.policyCompliant === true ? Number(policyDecision.exitPrice) : reference
  if (!positive(fillReference)) return rejected(position, 'REJECTED', ['Policy exit price is required'])
  ...
  const trade = { id: `paper-exit-${fingerprint.slice(0, 24)}`, symbol: position.symbol, assetType: position.assetType, side, orderType: 'market', quantity: requested, price: fillReference, paperTrading: true }
  ...
  const portfolio = { id: account.accountId ?? 'paper-portfolio', cash: Number(account.cash ?? 0), equity: Number(account.equity ?? account.cash ?? 0), realizedPnl: Number(account.realizedPnl ?? 0), positions: Array.isArray(positions) ? positions : [position] }
  const execution = simulateRealtimePaperExecution({ preparedTrades: [prepared], portfolio, quote: { last: fillReference, bid: fillReference, ask: fillReference, high: fillReference, low: fillReference, liquidityScore: quote.liquidityScore ?? 75, timestamp: observedAt }, paperTrading: true, liveOrders: false, brokerExecution: false }, { emitEvent: false, timestamp: now })
  ...
  const remaining = accounting.positions.find((value) => value.symbol === position.symbol && value.assetType === position.assetType && value.side === position.side) ?? null
  ...
  return {
    ...
    exitPlan: { positionId: position.positionId, symbol: position.symbol, strategyId: position.strategyId, existingSide: position.side, currentQuantity: Number(position.quantity), requestedExitQuantity: requested, remainingQuantity: remaining?.quantity ?? 0, averageCost: Number(position.averagePrice), referencePrice: fillReference, currentQuotePrice: reference, policyExitPrice: policyDecision?.exitPrice ?? null, simulatedExitPrice: item.executionSimulation?.fill?.fillPrice, fees: item.executionSimulation?.fill?.fees, slippageBps: item.executionSimulation?.fill?.slippageBps, slippageAmount: item.executionSimulation?.fill?.slippageAmount, feeModel: 'executionSimulationEngine.defaultFeeModel', slippageModel: 'executionSimulationEngine.defaultSlippageModel', realizedPnlDelta: accounting.account.realizedPnlDelta, evidenceTimestamp: policyDecision?.exitPrice ? policyBar.observedAt ?? policyBar.timestamp : observedAt, policyTrigger: policyDecision?.reason ?? null, paperTradingOnly: true, automaticExecution: false },
