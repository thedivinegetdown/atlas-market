...
export function simulatePaperPositionExit({ position, account = {}, quantity, quote = {}, existingExits = [], paperModeEnabled = true, exitReason, policyBar, sessionsHeld } = {}, options = {}) {
  ...
  const policyDecision = position.exitPolicy
    ? evaluateIndexPullbackExitPolicy({ policy: position.exitPolicy, bar: policyBar ?? { open: reference, high: reference, low: reference, close: reference, observedAt, freshness: 'FRESH' }, sessionsHeld })
    : null
