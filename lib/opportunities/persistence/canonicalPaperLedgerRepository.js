  ...
  return {
    ...
    async commitExit(input = {}) {
      ...
      return database.transaction(async (client) => {
        ...
        const result = simulatePaperPositionExit({ position: { ...position, exitPolicy: input.exitPolicy ?? null }, account, quantity: requestedQuantity, quote: input.quote, paperModeEnabled: input.paperModeEnabled === true, exitReason: input.exitReason, policyBar: input.policyBar, sessionsHeld: input.sessionsHeld }, { now: input.now })
        ...
        const forwardObservation = cohortKey && entryRows.rows.every((row) =>
          ...
          && row.payload?.exitPolicy?.definitionFingerprint === result.exitPolicy?.definitionFingerprint,
        ) ? entryCohort : null
