    ...
    async commitExit(input = {}) {
      ...
      return database.transaction(async (client) => {
        ...
        const entryRows = await client.query(
          `SELECT payload FROM atlas_paper_executions WHERE account_record_id=$1 AND position_id=$2 AND execution_type='entry' ORDER BY created_at ASC,id ASC`,
          [account.recordId, position.positionId],
        )
        const entries = (entryRows.rows ?? []).map((row) => row.payload)
        ...
        const entryCohort = entryRows.rows?.[0]?.payload?.forwardObservation
        const cohortKey = entryCohort?.experimentId === 'EDGE.2' ? JSON.stringify(entryCohort) : null
        const forwardObservation = cohortKey && entryRows.rows.every((row) =>
          JSON.stringify(row.payload?.forwardObservation) === cohortKey
          && row.payload?.exitPolicy?.fingerprint === result.exitPolicy?.fingerprint,
        ) ? entryCohort : null
