  ...
  return {
    ...
    async commitExit(input = {}) {
      ...
      return database.transaction(async (client) => {
        ...
        const result = simulatePaperPositionExit({ position: { ...position, exitPolicy }, account, quantity: requestedQuantity, quote: input.quote, paperModeEnabled: input.paperModeEnabled === true, exitReason: input.exitReason }, { now: input.now })
