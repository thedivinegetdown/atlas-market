...
export function executePaperReportJob(input = {}, options = {}) {
  const started = startPaperReportJob(input, { ...options, emitEvent: false }).paperReportJob
  try {
    if (started.jobType === 'audit-report-generation') {
      ...
      return completePaperReportJob({ paperReportJob: started, outputReferences: [audit.paperAuditReport.id] }, options)
    }
    ...
    if (started.jobType === 'export-generation') {
      ...
      return completePaperReportJob({ paperReportJob: started, outputReferences: [report.paperReport.id, exported.paperReportExport.id] }, options)
    }
    return completePaperReportJob({ paperReportJob: started, outputReferences: [report.paperReport.id] }, options)
  } catch (error) {
