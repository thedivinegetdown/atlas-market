...
const PAPER_ELIGIBLE_LIFECYCLES = new Set(['active', 'paper_forward_observation'])
...
function evaluateOne(strategyInput, regime, config) {
  ...
  if (blockingReasons.length > 0) decision = STRATEGY_SUITABILITY_DECISIONS.DISABLED
    ...
    || !PAPER_ELIGIBLE_LIFECYCLES.has(strategy.lifecycleState)
  ...
  if (!PAPER_ELIGIBLE_LIFECYCLES.has(strategy.lifecycleState)) reasons.push(`Strategy lifecycle remains ${strategy.lifecycleState}; selection cannot activate it`)
  if (strategy.lifecycleState === 'paper_forward_observation') reasons.push('Strategy is approved only for fixed paper forward observation; live trading is not approved')
