...
import { VOLATILITY_EXPANSION_STRATEGY_VERSION } from '../volatility/volatilityExpansionSignal.js'
import { VOLATILITY_EXPANSION_EXIT_POLICY_VERSION } from '../../opportunities/forwardTest/volatilityExpansionExitPolicy.js'
...
function implementedRecord(record = {}) {
  ...
  return {
    strategyId,
    familyId: strategyId === 'index-pullback-v1' ? 'trend-pullback' : strategyId === 'breakout-momentum-v1' ? 'breakout-momentum' : strategyId === 'range-mean-reversion-v1' ? 'range-mean-reversion' : strategyId === 'volatility-expansion-v1' ? 'volatility-expansion' : 'unclassified',
    displayName: record.strategyName ?? strategyId,
    version: record.versionReference ?? (strategyId === 'index-pullback-v1' ? INDEX_PULLBACK_STRATEGY_VERSION : strategyId === 'breakout-momentum-v1' ? BREAKOUT_MOMENTUM_STRATEGY_VERSION : strategyId === 'range-mean-reversion-v1' ? RANGE_MEAN_REVERSION_STRATEGY_VERSION : strategyId === 'volatility-expansion-v1' ? VOLATILITY_EXPANSION_STRATEGY_VERSION : null),
    ...
    exitPolicyId: strategyId === 'index-pullback-v1' ? INDEX_PULLBACK_EXIT_POLICY_VERSION : strategyId === 'breakout-momentum-v1' ? BREAKOUT_MOMENTUM_EXIT_POLICY_VERSION : strategyId === 'range-mean-reversion-v1' ? RANGE_MEAN_REVERSION_EXIT_POLICY_VERSION : strategyId === 'volatility-expansion-v1' ? VOLATILITY_EXPANSION_EXIT_POLICY_VERSION : null,
