...
export function buildDurablePaperWorkflowProjections({ account = {}, positions = [], executions = [], history, asOf } = {}) {
  const measurement = buildCanonicalPaperOutcomes(executions, { history })
  const performance = reviewPaperPerformance(measurement.outcomes, { asOf, cohortIsolation: true, equityChronology: measurement.equityChronology })
  const realizedValues = measurement.outcomes.map((item) => finite(item.netPnl))
  ...
  return {
    ...
    summary: {
      ...
      sharpeRatio: performance.performance.sharpeStyle ?? 0, maxDrawdown: performance.performance.maximumDrawdownPct, drawdownStatus: performance.performance.drawdownStatus, expectancy: performance.performance.expectancyPerTrade,
    ...
    outcomeMeasurement: { history: measurement.history, equityChronology: measurement.equityChronology, excludedOutcomes: measurement.excludedOutcomes, boundaries: measurement.boundaries },
