...
export function rankBriefingPriorities(model, config) {
  ...
  for (const opportunity of model.opportunities) {
    if (opportunity.freshness === 'STALE') items.push(priority(`opportunity-${opportunity.symbol}`, 'MEDIUM', 'Verify stale opportunity evidence', `${opportunity.symbol} quality evidence is stale.`, 'opportunities'))
    else if (opportunity.status === 'COMPLETE' && opportunity.score >= config.thresholds.highOpportunityScore) items.push(priority(`opportunity-${opportunity.symbol}`, 'HIGH', 'Review high-quality opportunity', `${opportunity.symbol} has a ${opportunity.band} advisory quality result.`, 'opportunities'))
