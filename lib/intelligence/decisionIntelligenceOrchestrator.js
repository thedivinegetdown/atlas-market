...
export async function buildDecisionIntelligence({ tenantContext = {}, accountId, selectedPlanId = null, evidenceRepository, ledgerRepository, evaluations, positions, account, executions, correlationEvidence = null, generatedAt = new Date().toISOString() } = {}) {
  ...
  const copilotContext = buildAtlasDecisionContext({ plans, ranking, portfolioAdmissions: admissions, strategyRegistry: registry, decisionQuality, selectedPlanId, generatedAt })
  ...
  return Object.freeze({
    ...
    market: { status: status(plans), freshness: plans.some((plan) => plan.market?.freshness === 'STALE') ? 'STALE' : plans[0]?.market?.freshness ?? 'UNAVAILABLE', provenance: plans[0]?.market?.provenance ?? null },
