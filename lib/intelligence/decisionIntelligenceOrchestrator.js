...
const assessment = (plan) => ({ strategyId: plan.strategyId, strategyVersion: plan.strategyVersion ?? null, strategyFamily: plan.strategyFamily ?? null, status: plan.decision?.status ?? 'INSUFFICIENT_DATA', suitability: plan.strategy?.suitability ?? 'UNKNOWN', reasons: bounded(plan.decision?.supportingReasons, 3), cautions: bounded(plan.decision?.cautionReasons, 3), blockers: bounded(plan.risk?.rejectionReasons, 3), noTradeReason: plan.decision?.noTradeReason ?? null, evidenceType: plan.strategyEvidence?.type ?? null, evidenceFreshness: plan.market?.freshness ?? 'UNKNOWN', strategyFingerprint: plan.integrity?.strategyFingerprint ?? null })
const unavailableAssessment = (strategy) => ({ strategyId: strategy.strategyId, strategyVersion: strategy.version ?? null, strategyFamily: strategy.familyId ?? null, status: 'INSUFFICIENT_DATA', suitability: 'UNKNOWN', reasons: [], cautions: [], blockers: ['No current evaluated candidate evidence is available.'], noTradeReason: 'No current evaluated candidate evidence is available.', evidenceType: null, evidenceFreshness: 'UNKNOWN', strategyFingerprint: strategy.strategyFingerprint ?? null })
...
export async function buildDecisionIntelligence({ tenantContext = {}, accountId, selectedPlanId = null, evidenceRepository, ledgerRepository, evaluations, positions, account, executions, correlationEvidence = null, marketContext = null, observationStatuses, observationStatusResolver = resolvePersistedForwardObservationStatuses, generatedAt = new Date().toISOString() } = {}) {
  ...
  const executionHistory = executions
    ? { executions, history: { status: 'PROVIDED', returnedCount: executions.length, latest: true } }
    : await ledgerRepository?.readExecutionHistory?.(scope)
      ?? { executions: await ledgerRepository?.listExecutions?.(scope) ?? [], history: { status: 'UNKNOWN', latest: null } }
  const loadedExecutions = executionHistory.executions
  const outcomeMeasurement = buildCanonicalPaperOutcomes(loadedExecutions, { history: executionHistory.history })
  ...
  const decisionQuality = buildDecisionQualityMonitor({ outcomes: outcomeMeasurement.outcomes, equityChronology: outcomeMeasurement.equityChronology, history: outcomeMeasurement.history, registry, generatedAt })
  ...
  const planAssessments = plans.map(assessment)
  const assessedStrategyIds = new Set(planAssessments.map((entry) => entry.strategyId))
  const strategyAssessments = bounded([...planAssessments, ...registry.strategies.filter((entry) => !assessedStrategyIds.has(entry.strategyId)).map(unavailableAssessment)], 20)
  const noTradeReasons = bounded(strategyAssessments.filter((entry) => entry.status === 'NO_TRADE').map((entry) => ({ strategyId: entry.strategyId, reason: entry.noTradeReason })), 8)
  const copilotContext = buildAtlasDecisionContext({ plans, ranking, portfolioAdmissions: admissions, strategyRegistry: registry, decisionQuality, marketContext, observationStatuses: resolvedObservationStatuses, strategyAssessments, selectedPlanId, generatedAt })
  return Object.freeze({
    ...
    opportunities: { topQualifiedPlans, watchPlans, qualifiedCount: ranking.qualified.length, watchCount: ranking.watch.length, blockedCount: admissions.filter((admission) => admission.admissionStatus === 'BLOCKED').length, emptyQualifiedState: topQualifiedPlans.length === 0 ? 'NO_QUALIFIED_OPPORTUNITIES' : null, noTradeReasons },
    ranking,
