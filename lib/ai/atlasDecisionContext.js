...
const planProjection = (plan = {}) => ({ planId: text(plan.planId), symbol: text(plan.symbol)?.toUpperCase() ?? null, side: text(plan.side), strategyId: text(plan.strategyId), decisionStatus: text(plan.decision?.status, 'INSUFFICIENT_DATA'), tradeQuality: { score: plan.quality?.score ?? null, band: text(plan.quality?.band, 'UNKNOWN') }, regime: plan.regime?.trendRegime ?? 'UNKNOWN', structure: { entry: plan.structure?.entry ?? null, stop: plan.structure?.stop ?? null, target: plan.structure?.target ?? null, maximumPlannedLoss: plan.risk?.maximumPlannedLoss ?? null }, rangeMeanReversion: plan.rangeMeanReversion ?? null, volatilityExpansion: plan.volatilityExpansion ?? null, reasons: [...(plan.decision?.supportingReasons ?? [])].slice(0, 4), cautionReasons: [...(plan.decision?.cautionReasons ?? [])].slice(0, 4), evidenceFingerprint: plan.integrity?.evidenceFingerprint ?? null })

export function buildAtlasDecisionContext({ plans = [], ranking = {}, portfolioAdmissions = [], strategyRegistry = {}, decisionQuality = null, marketContext = null, observationStatuses = [], selectedPlanId = null, generatedAt = new Date().toISOString() } = {}) {
  ...
  return Object.freeze({
    ...
    observations: (Array.isArray(observationStatuses) ? observationStatuses : []).slice(0, 8).map((entry) => ({ experimentId: entry.experimentId ?? entry.observationId ?? 'UNAVAILABLE', strategyId: entry.strategyId ?? 'UNAVAILABLE', status: entry.status ?? 'NOT_STARTED', sessionsElapsed: entry.sessionsElapsed ?? 0, completedOutcomes: entry.completedOutcomes ?? 0, minimumSessions: entry.minimumSessions ?? 20, minimumOutcomes: entry.minimumOutcomes ?? 30, blockers: [...(entry.blockers ?? [])].slice(0, 3) })),
