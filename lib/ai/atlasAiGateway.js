...
export function createAtlasAiRepository({ database } = {}) {
  return {
    ...
    async saveTradeQualityReview(input = {}) {
      const snapshot = normalizeTradeQualitySnapshot(input.qualitySnapshot ?? input)
      ...
      const id = `tq-${tenantContext.organizationId}-${accountId}-${userId}-${snapshot.opportunityId}`.slice(0, 500)
      ...
      await database.query(
        `INSERT INTO atlas_ai_opportunity_analysis_history
         ...
         WHERE atlas_ai_opportunity_analysis_history.organization_id = EXCLUDED.organization_id AND atlas_ai_opportunity_analysis_history.account_id = EXCLUDED.account_id AND atlas_ai_opportunity_analysis_history.user_id = EXCLUDED.user_id`,
        [id, tenantContext.organizationId, tenantContext.teamWorkspaceId ?? null, accountId, userId, `trade-quality-${userId}`, snapshot.symbol, snapshot.reviewState, reviewedAt, snapshot.asOf, [snapshot.opportunityId], snapshot.opportunityId, payload, expiresAt],
      )
    ...
    async savePaperEvaluation(input = {}) {
      ...
      await database.query(
        `INSERT INTO atlas_ai_opportunity_analysis_history
         ...
         ON CONFLICT (id) DO NOTHING`,
        [evaluation.evaluationId, tenantContext.organizationId, tenantContext.teamWorkspaceId ?? null, accountId, userId, `paper-evaluation-${userId}`, evaluation.symbol, evaluation.evaluatedAt, [evaluation.candidateId], evaluation.evidenceFingerprint, payload, input.expiresAt ?? null],
      )
      return { ok: true, evaluation, payload }
    },
    ...
    async savePaperSimulation(input = {}) {
      ...
      await database.query(`INSERT INTO atlas_ai_opportunity_analysis_history (id,organization_id,team_workspace_id,account_id,user_id,session_id,analysis_category,timeframe,symbol,ranking_tier,review_state,reviewed_by_user_id,reviewed_at,market_data_as_of,candidate_fingerprints,deterministic_baseline_ranks,advisory_ranking,excluded_candidates,no_trade_recommended,provider,model,prompt_version,context_fingerprint,latency_ms,usage_estimate,status,payload,created_at) VALUES ($1,$2,$3,$4,$5,$6,'paper_simulation','1D',$7,'review','reviewed',$5,$8,$8,$9,'[]'::jsonb,'[]'::jsonb,'[]'::jsonb,FALSE,'deterministic','guarded-paper-simulation-v1','guarded-paper-simulation-v1',$10,0,'{}'::jsonb,'completed',$11,NOW()) ON CONFLICT (id) DO NOTHING`,[`paper-simulation-${simulation.fingerprint}`,tenantContext.organizationId,tenantContext.teamWorkspaceId??null,accountId,userId,`paper-simulation-${userId}`,simulation.symbol,simulation.simulatedAt??new Date().toISOString(),[simulation.candidateId],simulation.fingerprint,payload])
      return {ok:true,simulation,payload}
    },
