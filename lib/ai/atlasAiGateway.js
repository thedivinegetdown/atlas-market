...
export function createAtlasAiRepository({ database } = {}) {
  return {
    ...
    async savePaperEvaluation(input = {}) {
      ...
      const write = await database.query(
        `INSERT INTO atlas_ai_opportunity_analysis_history
         ...
         ON CONFLICT (id) DO NOTHING
         RETURNING id`,
        [id, tenantContext.organizationId, tenantContext.teamWorkspaceId ?? null, accountId, userId, `paper-evaluation-${userId}`, evaluation.symbol, evaluation.evaluatedAt, JSON.stringify([evaluation.candidateId]), evaluation.evidenceFingerprint, payload, input.expiresAt ?? null],
      )
    ...
    async saveForwardEvidenceSnapshot(input = {}) {
      ...
      const current = await this.getForwardObservationManifest({ ...input, experimentId: snapshot.experimentId })
