...
import { analyzeOpportunityIntelligence } from './opportunityAnalysisEngine.js'
  ...
  const gateway = {
    async run(input = {}, options = {}) {
      ...
      if (requestCategory === 'opportunity_ranking' || requestCategory === 'market_overview' || requestCategory === 'trade_idea_analysis' || requestCategory === 'watchlist_prioritization' || requestCategory === 'market_regime_analysis' || requestCategory === 'candidate_comparison' || requestCategory === 'no_trade_analysis') {
        const opportunityAnalysis = await analyzeOpportunityIntelligence({
          ...input,
          requestCategory,
    ...
    async *stream(input = {}, options = {}) {
      ...
      yield createAtlasAiStreamEvent(ATLAS_AI_STREAM_EVENTS.started, { ...baseEvent, sequence: sequence += 1, metadata: { fallbackToNonStreaming: true } })
      const signal = options.signal ?? input.signal
      if (signal?.aborted) {
        const cancelled = createAtlasAiStreamEvent(ATLAS_AI_STREAM_EVENTS.cancelled, { ...baseEvent, sequence: sequence += 1, metadata: { reason: 'aborted_before_start' } })
      ...
      try {
        ...
        if (signal?.aborted) {
          const cancelled = createAtlasAiStreamEvent(ATLAS_AI_STREAM_EVENTS.cancelled, { ...baseEvent, sequence: sequence += 1, metadata: { reason: 'aborted_after_validation' } })
          eventBus?.emit?.(ATLAS_AI_EVENTS.streamCancelled, { ...cancelled, tenantScope: scope })
          yield cancelled
          return
        }
        ...
        for (let index = 0; index < streamText.length; index += config.streamChunkChars) {
          if (signal?.aborted) {
            const cancelled = createAtlasAiStreamEvent(ATLAS_AI_STREAM_EVENTS.cancelled, { ...baseEvent, sequence: sequence += 1, metadata: { reason: 'aborted_during_chunks' } })
            eventBus?.emit?.(ATLAS_AI_EVENTS.streamCancelled, { ...cancelled, tenantScope: scope })
            yield cancelled
            return
          }
          const chunk = createAtlasAiStreamEvent(ATLAS_AI_STREAM_EVENTS.chunk, { ...baseEvent, sequence: sequence += 1, chunk: streamText.slice(index, index + config.streamChunkChars) })
