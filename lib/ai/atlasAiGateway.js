  ...
  return {
    ...
    async createOpportunityAnalysisHistory(input = {}) {
      ...
      const safeHistory = {
        ...
        advisoryRanking: ranked.map((entry) => ({ opportunityId: entry.opportunityId, symbol: entry.symbol, advisoryRank: entry.advisoryRank, recommendation: entry.recommendation, confidence: entry.confidence })).slice(0, 20),
