...
const OPPORTUNITY_ANALYSIS_VERSION = 'atlas-opportunity-analysis-v2'
...
export async function analyzeOpportunityIntelligence(input = {}, options = {}) {
  ...
  const rankedCandidates = eligibleCandidates
    .sort((left, right) => right.baselineRank - left.baselineRank)
    .map((entry, index) => ({
      ...entry,
  ...
  const rankedOpportunities = rankedCandidates.map((entry, index) => ({
    ...
    rankChangeExplanation: sanitizeAiText(`Baseline rank ${entry.baselineRank} was preserved from deterministic Atlas eligibility and scanner order; advisory rank ${index + 1} reflects bounded AI interpretation only. This baseline explanation is required for rank changes.`, 900),
    reviewPriority: index === 0 ? 'high' : 'medium',
  }))
