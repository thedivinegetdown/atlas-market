...
const plannedLoss = (record = {}) => number(record.immutablePlannedRisk ?? record.qualifiedTradePlan?.risk?.maximumPlannedLoss ?? record.tradePlan?.risk?.maximumPlannedLoss ?? record.orderPlan?.maximumPlannedLoss)
const completed = (record = {}) => record.paperTradingOnly !== false && (record.status === 'SIMULATED_FILLED' || record.executionStatus === 'simulated' || record.decisionGate?.execution === 'filled') && (record.accountingStatus ?? record.accountingUpdate?.status ?? record.accountingUpdateSnapshot?.status) === 'position_closed' && number(record.netPnl ?? record.realizedPnl) != null
...
function rNormalized(records) {
  const values = records.map((record) => ({ outcome: number(record.netPnl ?? record.realizedPnl), risk: plannedLoss(record) })).filter((entry) => entry.outcome != null && entry.risk != null && entry.risk > 0).map((entry) => Number((entry.outcome / entry.risk).toFixed(4)))
...
export function buildDecisionQualityMonitor({ outcomes = [], performanceReview = null, equityChronology, history, registry = buildStrategyFamilyRegistry(), generatedAt = new Date().toISOString() } = {}, options = {}) {
  const comparable = outcomes.filter((record) => completed(record) && record.attribution?.status === 'COMPLETE' && record.cohortKey)
  const cohortKeys = [...new Set(comparable.map((record) => record.cohortKey))]
  const completedOutcomes = cohortKeys.length > 1 ? [] : comparable
  const review = performanceReview ?? reviewPaperPerformance(outcomes, { ...options, asOf: generatedAt, cohortIsolation: true, equityChronology })
  const learning = buildPaperLearningEvidence(review)
  const fingerprints = [...new Set(comparable.map((record) => record.attribution?.strategyFingerprint ?? record.qualifiedTradePlan?.integrity?.strategyFingerprint ?? record.strategyFingerprint ?? null).filter(Boolean))]
  const policyFingerprints = [...new Set(comparable.map((record) => record.attribution?.policyFingerprint ?? record.qualifiedTradePlan?.integrity?.policyFingerprint ?? record.policyFingerprint ?? null).filter(Boolean))]
  const experimentIds = [...new Set(comparable.map((record) => record.experimentId ?? record.forwardObservation?.experimentId ?? null).filter(Boolean))]
  const compatibilityStatus = cohortKeys.length > 1 ? 'INCOMPATIBLE_HISTORY' : cohortKeys.length === 1 ? 'COMPATIBLE' : 'UNAVAILABLE'
  ...
  return Object.freeze({
    ...
    overall: { completedOutcomes: review.sample?.completedTrades ?? 0, wins: review.sample?.wins ?? 0, losses: review.sample?.losses ?? 0, winRate: performance.winRate ?? 0, averageWin: performance.averageWin ?? 0, averageLoss: performance.averageLoss ?? 0, expectancy: performance.expectancyPerTrade ?? 0, grossPnL: performance.grossProfit ?? 0, netPnL: performance.netRealizedPnl ?? 0, profitFactor: performance.profitFactor ?? 0, largestWin: performance.largestWin ?? 0, largestLoss: performance.largestLoss ?? 0, drawdownStatus: performance.drawdownStatus ?? 'UNAVAILABLE', maxDrawdown: performance.maximumDrawdownPct ?? null, recoveryFactor: performance.recoveryFactor ?? null },
    ...
    compatibility: { status: compatibilityStatus, strategyFingerprints: fingerprints, policyFingerprints, cohortKeys, excludedMissingAttribution: outcomes.filter((record) => completed(record) && record.attribution?.status !== 'COMPLETE').length, reason: compatibilityStatus === 'INCOMPATIBLE_HISTORY' ? 'Outcomes with incompatible immutable fingerprints are excluded from pooled metrics.' : null },
    history: history ?? { status: 'UNKNOWN', latest: null },
