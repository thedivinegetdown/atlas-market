...
function createStore(database, storeName) {
  ...
  return {
    ...
    async upsertScoped(id, payload, tenantContext = {}) {
      ...
      const result = await database.query(
        `INSERT INTO ${tableName} (id, payload, organization_id, team_workspace_id, user_id, updated_at)
         ...
         ON CONFLICT (id) DO UPDATE SET payload = EXCLUDED.payload, organization_id = EXCLUDED.organization_id, team_workspace_id = EXCLUDED.team_workspace_id, user_id = EXCLUDED.user_id, updated_at = NOW()
    ...
    async listScoped({ limit = 50, organizationId, teamWorkspaceId = null } = {}) {
      if (!database.connected) return []
      const result = teamWorkspaceId
        ? await database.query(
          `SELECT id, payload, organization_id, team_workspace_id, user_id FROM ${tableName}
           WHERE organization_id = $1 AND team_workspace_id = $2
           ORDER BY updated_at DESC LIMIT $3`,
          [organizationId, teamWorkspaceId, limit],
        )
        : await database.query(
          `SELECT id, payload, organization_id, team_workspace_id, user_id FROM ${tableName}
           WHERE organization_id = $1 AND (team_workspace_id IS NULL OR team_workspace_id = '')
    ...
    async getScoped(id, tenantContext = {}) {
      ...
      const result = await database.query(
        `SELECT id, payload, organization_id, team_workspace_id, user_id FROM ${tableName}
         WHERE id = $1 AND organization_id = $2 AND COALESCE(team_workspace_id, '') = COALESCE($3, '')`,
