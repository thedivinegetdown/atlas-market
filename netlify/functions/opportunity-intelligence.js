...
export function createOpportunityIntelligenceHandler({ opportunityRepository, ...options } = {}) {
  return createOrganizationAuthenticatedApiHandler(async ({ event, body, query, tenantContext, user, membership }) => {
    ...
    try {
      const saved = await repository.saveTradeQualityReview({ ...context, qualitySnapshot: body.qualitySnapshot, reviewedAt: body.reviewedAt, expiresAt: body.expiresAt })
  ...
  }, { allowedMethods: ['GET', 'POST'], requiredPermission: 'dashboard.read', workspaceAction: 'read', routeId: 'opportunity-intelligence', maxRequestBytes: 16 * 1024, ...options })
}
