...
Several core repositories—legacy orders, portfolio/accounting, journal, alerts, and scanner state—still use process-memory arrays and are neither durable nor consistent across serverless instances. PI.2 now wires eligible reviewed Trade Quality evidence and PA.1/PA.2 evidence to the authenticated Function's DB.1 PostgreSQL adapter, with fail-closed behavior, tenant-scoped ids, and database-level retry suppression. Other SQL-capable AI/release repositories may still require explicit wiring. The detailed inventory and production contract are in `docs/persistence/PRODUCTION_PERSISTENCE_ARCHITECTURE.md`.

PI.1 and PI.2 are recorded in `docs/persistence/PAPER_WORKFLOW_PERSISTENCE_GAP_ANALYSIS.md`. The canonical intelligence path is Reviewed Opportunity → PA.1 → PA.2. The legacy `submit-paper-order` path is compatibility-only. PA.2 still begins from the existing process-local/default portfolio summary, daily quota is not a distributed atomic control, and durable intent evidence and the paper-position compatibility projection are separate writes. Atlas therefore does not yet have the PI.3 canonical account/execution ledger that preserves cash, cost basis, realized P&L, and atomic entry/exit linkage across instances.
...
6. PI.2 makes reviewed opportunity and PA.1/PA.2 evidence durable-capable and retry-safe through the existing PostgreSQL adapter, but the paper workflow still lacks the PI.3 canonical transactional account/execution/position ledger. Deployed database execution, capacity, migration ownership, backup, restore, retention, and RPO/RTO remain unverified.
