...
## EDGE.2 deterministic observation-exit update — 2026-08-25

`index-pullback-v1` version `1.2.0` is promoted only to `paper_forward_observation`; live trading remains explicitly unapproved. The versioned `index-pullback-exit-v1.0.0` policy freezes the existing entry stop and 2R target, applies a stop-first same-bar rule, prices adverse stop gaps at the open, caps favorable target gaps at target, and closes at session 20 if no price exit occurs. PA.2 attaches the immutable policy, PI.3 retains it in the execution ledger, PA.4 enforces full deterministic exits (with a separately attributed emergency-close path), and PA.3/PA.5 retain realized attribution. A definition change invalidates the fixed cohort. Production cohort start still requires an authenticated bounded market-hours check with genuine fresh provider, regime, scanner/TQ, and risk evidence for the approved five-symbol universe.
