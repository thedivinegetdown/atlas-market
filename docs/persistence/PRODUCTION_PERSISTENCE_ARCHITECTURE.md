...
| Migrations | The repository contains 69 unique, lexically ordered migrations tracked in `atlas_schema_migrations`. Each unapplied migration runs inside a transaction and failures propagate. Destructive SQL is prohibited by release verification. |
...
| Portfolio/accounting | Process-local repository arrays and computed provider summaries | Process-local only | No canonical durable account boundary | None | Not a durable ledger; production blocker for accounting claims |
...
The highest-value unresolved multi-write boundary is paper-order simulation: saving a filled simulation and saving its paper-position aggregate are separate operations and, in the current default path, use different repository abstractions. DB.1 does not claim atomicity and does not alter paper-trading logic. Release workflows with create-plus-activity calls have the same independent-write pattern unless their caller explicitly supplies a transaction.
